...
uint64_t zobrist_castle(int castling_rights) {
    uint64_t hash = 0;
...
uint64_t zobrist_enpassant(int sq) { return polyglot_random[772 + (sq % 8)]; }

uint64_t piece_index(uint8_t piece) {
    switch (piece) {
    ...
    case PIECE_KING | PIECE_WHITE:
        return 11;
    default:
        fprintf(stderr, "Invalid piece (%d) in piece_index\n", piece);
        exit(EXIT_FAILURE);
    }
}

uint64_t zobrist_piece(uint8_t piece, int sq) {
    int index = piece_index(piece);
    if (index < 0 || index >= 12 || sq < 0 || sq >= 64) {
        fprintf(stderr, "Invalid piece (%d) or square (%d) in zobrist_piece\n",
                piece, sq);
        exit(EXIT_FAILURE);
    }
    return polyglot_random[64 * index + sq];
}
#define ADD_PIECE_COLOR(piece, color)                                          \
    FOREACH_SET_BIT(p->bitboards[(piece) | (color)], sq) {                     \
        hash ^= zobrist_piece((piece) | (color), sq);                          \
    }

#define ADD_PIECE(piece)                                                       \
    do {                                                                       \
        ADD_PIECE_COLOR(piece, PIECE_WHITE);                                   \
        ADD_PIECE_COLOR(piece, PIECE_BLACK);                                   \
    } while (0)

uint64_t position_zobrist(Position *p) {
    uint64_t hash = 0;

    ADD_PIECE(PIECE_PAWN);
    ADD_PIECE(PIECE_KNIGHT);
    ADD_PIECE(PIECE_BISHOP);
    ADD_PIECE(PIECE_ROOK);
    ADD_PIECE(PIECE_QUEEN);
    ADD_PIECE(PIECE_KING);

    hash ^= zobrist_castle(p->castling_rights);
    int side_to_move = p->moves % 2 == 0 ? PIECE_WHITE : PIECE_BLACK;
    if (p->en_passant != 0) {
        int sq = __builtin_ctzll(p->en_passant);
        int file = sq % 8;
        int rank = sq / 8;
        int pawn_rank = (side_to_move == PIECE_WHITE) ? rank - 1 : rank + 1;
        uint64_t pawn_bb = p->bitboards[PIECE_PAWN | side_to_move];
        uint64_t mask = 0ULL;
        if (file > 0) {
            mask |= 1ULL << (8 * pawn_rank + (file - 1));
        }
        if (file < 7) {
            mask |= 1ULL << (8 * pawn_rank + (file + 1));
        }
