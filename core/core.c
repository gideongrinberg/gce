 ...
 * in get_legal_moves.
 */
#define ADD_PAWN_MOVE(from, to) do { \
...
int get_legal_moves(Board *board, uint32_t *arr) {
    ...
    for (int rank = 0; rank < 8; rank++) {
        for (int file = 0; file < 8; file++) {
            ...
            if (GET_COLOR(piece) == EMPTY || GET_COLOR(piece) != color) continue;
            switch (GET_TYPE(piece)) {
                case PAWN:
                    ...
                    if ((target & 0x88) == 0 && board->board[target] == EMPTY) {
                        ...
                        target = idx + (sign * 32);
                        bool can_double_push = (color == PIECE_WHITE && SQ_RANK(idx) == 1) || (color == PIECE_BLACK && SQ_RANK(idx) == 6);
                        if (can_double_push && (target & 0x88) == 0 && board->board[target] == EMPTY) {
